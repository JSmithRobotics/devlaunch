...
pub enum LaunchNotice {
    ...
    /// A profile was bound in as the container's Claude configuration.
    ///
    /// Said because attribution is the failure profiles exist to prevent: this is the
...
///
/// [`forwarded_claude`] sends a profile's access token by value, and that token
/// cannot refresh: there is nothing to refresh with and nowhere to persist the
/// result, so a session outliving its expiry asks the operator to log in. Binding
/// the profile *directory* instead gives Claude Code the same file it has on the
/// host, so it refreshes exactly as it does there and the refreshed credential
/// lands back in the profile.
///
/// The directory and not `.credentials.json`: a refresh rewrites that file, writes
/// like that are ordinarily atomic, and a bind of a file does not survive its
/// source being replaced by rename — the staleness this repo already measured
/// (`init-host.sh`). A bind of a directory follows the rename.
///
/// # Only when asked
///
/// [`Self::NotAsked`] is the whole of a launch that named no profile, and it emits
/// nothing. Redirecting `CLAUDE_CONFIG_DIR` unconditionally would take a
/// devcontainer's own deliberately-mounted configuration away from it and break
/// instruction sharing it had set up, for launches that never asked for a profile.
/// So the flag always works and nothing else changes behaviour.
///
/// A sum rather than an `Option<PathBuf>` for [`PixiCache`]'s reason: each way of
/// failing names a different state and both have to be reportable.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum ClaudeProfileMount {
    /// No `--claude-profile`, so nothing to bind and nothing to redirect.
    NotAsked,
    /// A name `ProfileName::parse` rejects, or a host that resolved no profiles root.
    NotAName { name: String },
    /// The name is one and the directory is not there.
...
impl ClaudeProfileMount {
    ...
    pub(crate) fn ensure(host: &Host) -> Self {
        ...
        // The credential and not just the directory. Binding a profile directory that
        // holds no login would hand the container a logged-out configuration and call it
        // success -- and the refusals `resolve_token` already builds for exactly this
        // (`NoCredential`, naming the directory the operator made) are better than that.
        // Falling through is what reaches them.
        if !crate::clients::claude::has_credential(&source) {
            return Self::Missing {
                name: named.to_owned(),
                source,
            };
    ...
    ///
    /// At least two, and the first two are not optional: a bind at a path Claude Code
    /// does not read is a directory nothing opens. The mount lands only at creation and
    /// the env is re-applied on every `up`, which is why the target is a constant — the
    /// source is the only half that varies, so switching profiles is a `recreate` and
    /// never a state where the two disagree.
    ///
    /// Everything after those two is [`dangling_symlink_binds`] -- see there for what a
    /// profile's top-level symlinks need and why. It has to come *after* the parent
    /// mount: nested binds land in the order they are given, and a per-link mount is
    /// only useful shadowing a link the parent bind already put in place. This is why
    /// the guard below asserts order and not just membership.
        ...
        match self {
            Self::Bound { source, .. } => {
                let mut args = vec![
                    "--mount".to_owned(),
                    format!(
                        "type=bind,source={},target={CLAUDE_CONFIG_TARGET}",
                        source.display()
                    ),
                    "--workspace-env".to_owned(),
                    format!("CLAUDE_CONFIG_DIR={CLAUDE_CONFIG_TARGET}"),
                ];
                args.extend(dangling_symlink_binds(source));
                args
            }
            Self::NotAsked | Self::NotAName { .. } | Self::Missing { .. } => Vec::new(),
        }
    }

    /// Whether this launch binds a profile in as the container's configuration.
    ///
    /// Test-only for now, and kept rather than inlined because it is the predicate the
    /// one open question needs: suppressing `ClaudeProfileNotForwarded` on a container
    /// that *was* created with a profile bound. That cannot be asked of the host --
    /// see `a_bindable_profile_does_not_stop_the_token_being_forwarded` for why -- so it
    /// waits on the probe's `claudedir` reaching this decision.
    #[cfg_attr(not(test), allow(dead_code))]
...
///
/// A profile is mostly symlinks into the operator's real `~/.claude` --
/// `CLAUDE.md`, `agents`, `skills`, `hooks`, and the rest -- and those targets
/// are absolute paths into a home the container does not have. The parent bind
/// above puts the *link* in the container, dangling; a bind of the link's
/// resolved target at the same position shadows the link with a real directory
/// and is never followed.
///
/// The alternative is one bind of the whole operator `~/.claude` at its own
/// absolute path, which makes every link resolve for free and is wrong: that
/// directory's `.credentials.json` is the *default* profile's login, so binding
/// it in would smuggle a second account's credential into a container
/// `--claude-profile` was asked to keep it out of. The guard against it is
/// `the_credential_mount_is_never_readonly_and_the_whole_claude_directory_is_never_mounted`,
/// which asserts on what must be absent from the argv rather than on what is
/// present, because a set-membership test passes with both mounts in place.
///
/// Each of these is `readonly`. The per-profile credential is the one thing a
/// container legitimately writes back (a refresh), and that file lives under
/// the parent bind, never under one of these -- so making these read-only
/// costs nothing a refresh needs and shrinks what a workspace can reach on the
/// host to exactly the profile it was given.
///
...
///
/// Three things make a top-level entry emit nothing here, and each is silent
/// rather than a warning, because a `dl` launch is not the place to relitigate
/// how the operator built their `~/.claude`:
/// - it is not a symlink (a real file or directory belongs to the profile
///   itself and the parent bind already carries it);
/// - it is a relative link that resolves to somewhere still inside the
///   profile (again, the parent bind already carries it); or
/// - its target does not exist on the host, because binding a missing source
///   is how devpod fails the *entire* launch, and one stale link must not cost
///   a workspace the ability to open at all.
fn dangling_symlink_binds(profile: &Path) -> Vec<String> {
    // The profile not resolving at all is a state `ensure` has already ruled out
    // for `Bound` (it requires a directory holding a credential) -- but this
    // walks a filesystem a moment after that check ran, so treating a surprise
    // failure here as "nothing to add" rather than panicking costs nothing.
    ...
    for name in names {
        ...
        // `symlink_metadata` does not follow the link, unlike `metadata` --
        // this is what lets a real directory or file be told apart from a
        // symlink instead of being read as whatever it points at.
        ...
        args.push(format!(
            "type=bind,source={},target={CLAUDE_CONFIG_TARGET}/{},readonly",
            resolved.display(),
...
fn up_under_stage(
    ...
    let token_args = staged
        ...
        .unwrap_or_default();
    // Said here and not in `forwarded_claude`, because this is the path that creates a
    // container: it is the only place that knows the mount actually landed.
    if let ClaudeProfileMount::Bound { name, source } = &claude_mount {
        notices.say(LaunchNotice::ClaudeProfileBound {
            name: name.clone(),
            source: source.clone(),
        });
    }
    let args = up_args(request, &options, &pixi, &claude_mount, &token_args);
...
impl<'a> SessionContext<'a> {
    ...
    fn forwarded_claude(
        ...
        // Deliberately unchanged by the profile mount, and the reason is a hole the
        // first draft of this fell into: `ClaudeProfileMount::ensure` answers whether
        // the *host* has a bindable profile, not whether the container was created with
        // it bound. A mount lands only at creation, so for a workspace that is already
        // running -- which is most launches -- there is nothing bound and forwarding is
        // the only mechanism there is. Skipping it here on `ensure`'s answer made a
        // named profile silently do nothing against a running workspace, which is the
        // outcome the notice below exists to prevent.
        //
...
mod tests {
    ...
    fn a_launch_that_names_no_profile_binds_nothing() {
        // The whole of the conservative default. Redirecting `CLAUDE_CONFIG_DIR` for a
        // launch that asked for no profile would take a devcontainer's own mounted
        // configuration away from it, and break instruction sharing it had set up, for
        // nothing anyone requested.
        let scene = Scene::new();
    ...
    fn a_named_profile_is_bound_and_the_config_dir_points_at_it() {
        ...
        let mount = ClaudeProfileMount::ensure(&host);
        assert!(mount.is_bound(), "{mount:?}");
        // Both flags, and the second is the load-bearing one: a bind at a path Claude
        // Code does not read is a directory nothing opens.
        assert_eq!(
            mount.up_args(),
            vec![
                "--mount".to_owned(),
    ...
    /// A profile built to look like the real one the plan describes: three
    /// top-level entries that dangle (an absolute symlink to a directory, one to
    /// a file, and one whose target has been removed), one that does not (a
    /// relative link staying inside the profile), and the credential itself,
    /// which is a real file and never a link.
    struct SymlinkedProfile {
        _operator_home: tempfile::TempDir,
        _profiles_root: tempfile::TempDir,
        profile: PathBuf,
        agents: PathBuf,
    }

    fn a_profile_with_dangling_symlinks() -> SymlinkedProfile {
        let operator_home = tempfile::tempdir().expect("a fake operator home");
        let dot_claude = operator_home.path().join(".claude");
        let agents = dot_claude.join("agents");
        std::fs::create_dir_all(&agents).expect("the operator's agents directory");
        std::fs::write(dot_claude.join("CLAUDE.md"), "instructions").expect("the operator's file");

        let profiles_root = tempfile::tempdir().expect("a scratch profiles root");
        let profile = profiles_root.path().join("bear");
        std::fs::create_dir_all(&profile).expect("the profile directory");
        std::fs::write(profile.join(".credentials.json"), "{}").expect("a credential");

        // Dangles: an absolute link to a directory outside the profile.
        std::os::unix::fs::symlink(&agents, profile.join("agents")).expect("the agents link");
        // Dangles: an absolute link to a file outside the profile.
        std::os::unix::fs::symlink(dot_claude.join("CLAUDE.md"), profile.join("CLAUDE.md"))
        ...
        // Does not dangle because there is nothing to bind: the target was removed.
        std::os::unix::fs::symlink(dot_claude.join("gone"), profile.join("gone"))
            .expect("a dangling-target link");

        SymlinkedProfile {
            _operator_home: operator_home,
            _profiles_root: profiles_root,
            profile,
            agents,
        }
    }
    ...
    fn a_dangling_top_level_symlink_gets_its_own_readonly_mount() {
        let scene = a_profile_with_dangling_symlinks();
        let args = bound_mount_for(&scene.profile).up_args();

        let agents_target = format!("{CLAUDE_CONFIG_TARGET}/agents");
        let expected = format!(
            "type=bind,source={},target={agents_target},readonly",
            scene.agents.display()
        );
        assert!(
    ...
    #[test]
    fn the_credential_mount_is_never_readonly_and_the_whole_claude_directory_is_never_mounted() {
        // The two things a membership test on the argv would miss: nothing here
        // may say the credential mount is read-only, and nothing may name the
        // operator's whole `~/.claude` as a mount source, which would put the
        // *default* profile's login into a container launched as a different one.
        let scene = a_profile_with_dangling_symlinks();
        let args = bound_mount_for(&scene.profile).up_args();

        let dot_claude = scene
            .agents
            .parent()
            .expect("agents lives directly under .claude")
            .to_path_buf();
        // Exact-match the mount-source clause rather than substring-searching for
        // the path, since every legitimate mount here (e.g. `.../.claude/agents`)
        // has `.../.claude` as a *prefix* of its source and would otherwise trip
        // a substring check that was never asking the right question.
        let whole_claude_source = format!("source={},target=", dot_claude.display());
        assert!(
            !args.iter().any(|arg| arg.contains(&whole_claude_source)),
            "the operator's whole .claude directory must never be a mount source: {args:?}"
        );

        for arg in &args {
            if arg.contains(".credentials.json") {
                assert!(
    ...
    #[test]
    fn a_profile_directory_that_is_not_there_binds_nothing() {
        // The launch survives, as it does for a pixi cache that could not be made: a
        // missing profile costs the binding, and `resolve_token`'s refusal is what
        // reports the name -- this type does not get to fail a launch on its own.
        let root = tempfile::tempdir().expect("a scratch profiles root");
        let host = Host {
            claude: crate::clients::claude::HostEnv {
                profile: Some("absent".to_owned()),
                ..Default::default()
            },
            claude_profiles_root: Some(root.path().to_path_buf()),
            ..Scene::new().host
        ...
        let mount = ClaudeProfileMount::ensure(&host);
        assert!(
            matches!(mount, ClaudeProfileMount::Missing { .. }),
            "{mount:?}"
        );
        assert_eq!(mount.up_args(), Vec::<String>::new());
        assert!(!mount.is_bound());
    ...
    #[test]
    fn a_name_that_is_not_a_name_binds_nothing() {
        // `ProfileName::parse` is the one gate on what a profile may be called, and a
        // path traversal reaching a `--mount` source is the reason it exists. Refused
        // here by asking it rather than by a second rule.
        let root = tempfile::tempdir().expect("a scratch profiles root");
        let host = Host {
            claude: crate::clients::claude::HostEnv {
                profile: Some("../../etc".to_owned()),
                ..Default::default()
            },
            claude_profiles_root: Some(root.path().to_path_buf()),
            ..Scene::new().host
        ...
        let mount = ClaudeProfileMount::ensure(&host);
        assert!(
            matches!(mount, ClaudeProfileMount::NotAName { .. }),
            "{mount:?}"
        );
        assert_eq!(mount.up_args(), Vec::<String>::new());
    }
    ...
    #[test]
    fn a_bindable_profile_does_not_stop_the_token_being_forwarded() {
        // The hole the first draft of this fell into, kept as a test because it is not
        // obvious and it silently did nothing.
        //
        // `ClaudeProfileMount::ensure` answers about the *host*: this profile could be
        // bound. It does not say the container was created with it, and a mount lands
        // only at creation -- so against an already-running workspace, which is most
        // launches, nothing is bound and the forwarded token is the only mechanism
        // there is. Suppressing the forward on `ensure`'s answer made `--claude-profile`
        // against a running workspace do nothing at all, in silence.
        let root = tempfile::tempdir().expect("a scratch profiles root");
        let profile = root.path().join("bear");
        std::fs::create_dir_all(&profile).expect("the profile directory");
        std::fs::write(profile.join(".credentials.json"), "{}").expect("a credential");
        ...
        // Bindable, so the first draft would have returned early here.
        assert!(ClaudeProfileMount::ensure(&host).is_bound());
        ...
        let forwarded = context.forwarded_claude(&mut notices);
        assert!(
            forwarded.is_err() || forwarded.expect("checked").is_some(),
            "a bindable profile must still resolve a token, or refuse: {notices:?}"
        );
    }
